...
GPT-5.6 Sol/Terra/Luna(`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`)也会作为 documented
addition 准备在这条原生路径中出现,但**只有拥有 preview 权限时**才能调用。opencodex 会将这三个模型
广告为 372,000 usable token 上下文,并提供 `max` reasoning tier。
...
**OpenAI (API key)** provider 使用 `openai-responses` adapter 和 `https://api.openai.com/v1`。
fallback 模型列表包含 `gpt-5.5` 以及 preview-gated 的 `gpt-5.6-sol`、`gpt-5.6-terra`、
`gpt-5.6-luna`,三款 GPT-5.6 模型的 usable context 均为 372,000 tokens。**OpenRouter** 也会以
fallback seed 形式包含 `openai/gpt-5.6-sol`、`openai/gpt-5.6-terra`、`openai/gpt-5.6-luna`。
这两条路径都需要相应上游账号具备 preview 权限。
