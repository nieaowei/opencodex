...
GPT-5.6 Sol/Terra/Luna(`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`)는 이 native 경로에도
documented addition으로 준비되어 있지만 **preview 권한이 있을 때만** 호출됩니다. opencodex는 이 세
모델을 372,000 usable token 컨텍스트와 `max` reasoning tier로 광고합니다.
...
**OpenAI (API key)** 프로바이더는 `openai-responses` 어댑터와 `https://api.openai.com/v1`을
사용합니다. fallback 모델 목록은 `gpt-5.5`와 preview-gated `gpt-5.6-sol`, `gpt-5.6-terra`,
`gpt-5.6-luna`이며, GPT-5.6 세 모델의 usable context는 372,000 tokens로 설정됩니다.
**OpenRouter**도 fallback seed로 `openai/gpt-5.6-sol`, `openai/gpt-5.6-terra`,
`openai/gpt-5.6-luna`를 포함합니다. 두 경로 모두 해당 업스트림 계정에 preview 권한이 있어야
성공합니다.
